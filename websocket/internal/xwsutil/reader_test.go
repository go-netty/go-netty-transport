...
func TestReaderNoFrameAdvance(t *testing.T) {
	...
	if _, err := r.Read(make([]byte, 10)); err != wsutil.ErrNoFrameAdvance {
		t.Errorf("Read() returned %v; want %v", err, wsutil.ErrNoFrameAdvance)
	}
}
...
func TestReaderNextFrameAndReadEOF(t *testing.T) {
	for _, test := range []struct {
	...
	}{
		{
			...
			nextFrameErr: io.EOF,
			readErr:      wsutil.ErrNoFrameAdvance,
		},
		{
			source: func() io.Reader {
...
func TestMaxFrameSize(t *testing.T) {
	...
	_, err := r.NextFrame()
	if got, want := err, wsutil.ErrFrameTooLarge; got != want {
		t.Errorf("NextFrame() error = %v; want %v", got, want)
	}

	p := make([]byte, 100)
	n, err := r.Read(p)
	if got, want := err, wsutil.ErrNoFrameAdvance; got != want {
